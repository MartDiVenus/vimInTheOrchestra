...
## Intro: Bash script about netrw based Disk Manager. {{{1
...
touch /tmp/valeNetrw-devMontati0

rm -fr ~/.vim/plugin/Valentine/netrw/commands/valeparted

mkdir ~/.vim/plugin/Valentine/netrw/commands/valedisks 2> /dev/null

cp /usr/local/share/valentine/diskManager/plugins/* ~/.vim/plugin/Valentine/netrw/commands/valedisks

sudo cp /usr/local/share/valentine/diskManager/autoload/netrw.vim.disk /usr/share/vim/vim90/autoload/netrw.vim

cp /usr/local/share/valentine/diskManager/plugin/Valentine.vim.disk ~/.vim/plugin/Valentine/Valentine.vim

sudo rm -r /etc/vim

sudo cp -r /usr/local/share/valentine/diskManager/etc/vim /etc
...
# cat /tmp/valeNetrw-devMontati1 | sed 's/├─//' > /tmp/valeNetrw-devMontati2
 ...
 cat /tmp/valeNetrw-devMontati1 | sed 's/├─//' > /tmp/valeNetrw-devMontati1b

 cat /tmp/valeNetrw-devMontati1b | sed 's/│//' > /tmp/valeNetrw-devMontati2
...
cat /tmp/valeNetrw-dischi3 | sed '/^└─/d' > /tmp/valeNetrw-dischi4a

cat /tmp/valeNetrw-dischi4a | sed '/^│/d' > /tmp/valeNetrw-dischi4
...
cat /tmp/valeNetrw-partizioni2 | sed 's/└─//g' > /tmp/valeNetrw-partizioni2b

cat /tmp/valeNetrw-partizioni2b | sed 's/│//g' > /tmp/valeNetrw-partizioni3
   ...
   do
    ...
    if test $leggoBytes -gt 0
     ...
     then
      ...
      mkdir /tmp/ValeDisks/$leggoA/$leggoB 2> /dev/null
		   
      sudo blkid /dev/$leggoB > /tmp/ValeDisks/$leggoA/$leggoB/info

      echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/info

      ## No df per dischi non montati perché non sarebbe veritiero il responso.
      ## df -h /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/info
    ...
    if [ -f /tmp/valeNetrw-partMontate2 ]; then
     ...
     for c in $(ls /tmp/valeNetrw-splitDevMontati/)
	  ...
	  then
	   ...
	   echo "boot" > /tmp/ValeDisks/$leggoA/$leggoB/boot

	   echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/boot

	   sudo blkid /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/boot
                                
	   echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/boot
				
	   df -h /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/boot
                  
	   rm -f /tmp/ValeDisks/$leggoA/$leggoB/info
 	 ...
 	 if test "$leggoDeviceRadice" == "$leggoB"
	   ...
	   then
            ...
            echo "radice" > /tmp/ValeDisks/$leggoA/$leggoB/radice
	    
	    echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/radice

	    sudo blkid /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/radice

	    echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/radice

	    df -h /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/radice
	 ...
	 if test "$leggoDevPWD" == "$leggoB"
	  ...
	  then
	    ...
	    echo "corrente posizione del terminale (\$PWD)" > /tmp/ValeDisks/$leggoA/$leggoB/pwd

	    echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/pwd

	    sudo blkid /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/pwd

	    echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/pwd

            df -h /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/pwd
	...
	if test "$leggoB" == "$leggoD"
	 ...
	 then
	  ...
	  leggoMountedPtoB=$(cat /tmp/valeNetrw-mountedPtoB)
          ...
          if [ -d /tmp/ValeDisks/$leggoA/$leggoB ]; then
			       
 	   ## echo "pto. di mount: $leggoMountedPtoB" > /tmp/ValeDisks/$leggoA/$leggoB/mounted

	   ## echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/mounted

	   sudo blkid /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/mounted

	   echo " " >> /tmp/ValeDisks/$leggoA/$leggoB/mounted

           df -h /dev/$leggoB >> /tmp/ValeDisks/$leggoA/$leggoB/mounted

	   cat /tmp/ValeDisks/$leggoA/$leggoB/mounted | sort > /tmp/valeNetrw-mountedSort
	   ...
	   cat /tmp/valeNetrw-mountedHeader /tmp/valeNetrw-mountedHead3 > /tmp/ValeDisks/$leggoA/$leggoB/mounted
	  ...
	  ### per non avere ripetizioni 
	   ...
	   rm -f /tmp/ValeDisks/$leggoA/$leggoB/pwd
...
cat /tmp/valeNetrw-ValeDiskTreePunti | sed 's/\./ /g' > /tmp/valeNetrw-ValeDisksTree

#vim -c /|_/\-\-\/dev/g /tmp/valeNetrw-ValeDisksTree -c :w -c :q

## notazione scientifica delle cifre decimali
vim -c :%s/\,/\./g /tmp/valeNetrw-ValeDisksTree -c :w -c :q


grep "|--" /tmp/valeNetrw-ValeDisksTree > /tmp/valeNetrw-pipeTrattini
...
do
	...
	grep $leggoChiaveE /tmp/valeNetrw-ValeDisksTree > /tmp/valeNetrw-bassi
...
mv /tmp/valeNetrw-star /tmp/valeNetrw-ValeDisksTree 
...
cat /tmp/valeNetrw-ValeDisksTree
...
echo "If you want to get more details about one or more partitions:
you should mount or remount them, and you should restart ValeDisks.

Disks having partitions tables but no partitions, are not printed on stdout;
these disks are displayed on ValeDisks' window only.
...
gvim -f -geometry 20x30 /tmp/ValeDisks/
...
### /tmp/luxValeNetrwHacklux-SMarked 
## e /tmp/luxValeNetrwHacklux-DMarked
## servono per le operazioni esterne a valedisks
cp /tmp/valeNetrw-SMarked /tmp/luxValeNetrwHacklux-SMarked 2> /dev/null

cp /tmp/valeNetrw-DMarked /tmp/luxValeNetrwHacklux-DMarked 2> /dev/null
