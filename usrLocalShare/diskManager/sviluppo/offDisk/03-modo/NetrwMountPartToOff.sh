...
rm -fr /tmp/valeNetrw-Tmp

mkdir /tmp/valeNetrw-Tmp

ptoMontaggio="/tmp/valeNetrw-Tmp"
...
## /home/mart/.vim/plugin/Valentine/netrw/commands/NetrwMountPart.vim
		  #ptoMontaggio="$(cat /tmp/valeNetrw-mountingPoint)"
	   	### questa istanza [avente le opzioni umask, uid] non vale per tutti i file system, i.e. è valida per ext3 ma non per xfs
                ## sudo mount -o rw,users,nosuid,nodev,nofail,umask=000,uid=1000 /dev/$leggoPartizione $ptoMontaggio
	        ##	  
                ### questa istanza invece è universale ma per vfat è richiesta umask=000,uid=1000
	        #sudo mount -o rw,users,nosuid,nodev,nofail /dev/$leggoPartizione $ptoMontaggio
...
Premi Enter per chiudere."
		  ...
		  echo "$ptoMontaggio" > /tmp/ValeDisks/$leggoDevice/$leggoPartizione/mounted

		  echo " " >> /tmp/ValeDisks/$leggoDevice/$leggoPartizione/mounted

		  sudo blkid /dev/$leggoPartizione >> /tmp/ValeDisks/$leggoDevice/$leggoPartizione/mounted

		  echo " " >> /tmp/ValeDisks/$leggoDevice/$leggoPartizione/mounted

                  df -h /dev/$leggoPartizione >> /tmp/ValeDisks/$leggoDevice/$leggoPartizione/mounted
		  ...
		  if test $leggoType == "ntfs"
		   then
			sudo ntfsfix /dev/$leggoPartizione
                        ...
                        sudo mount -t ntfs-3g -o rw,users,nosuid,nodev,nofail,remove_hiberfile,umask=000,uid=1000 /dev/$leggoPartizione $ptoMontaggio
