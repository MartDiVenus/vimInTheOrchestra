...
read -p "mart: Ti copierò nella iso (lettura e scrittura) i file espressi con
l'opzione --scl, e che puoi visionare in /tmp/valeparted-listOfFilesToCopy.
Tale copia potrebbe richiedere del tempo.

Se i file da copiare fossero presente in una partizione non di sistema,
prima di procedere premendo Enter, dovresti montarla manualemnte e senza
ricorrere alle partizioni statiche.
I file devono rispettare i percorsi presenti in /tmp/valeparted-listOfFilesToCopy.


Alla fine della copia ti lascerò montato il file *.iso 
affinché tu possa verificare e/o caricarlo di ulteriori contenuti.
...
fileToCopy="$(cat /tmp/valeparted-listOfFilesToCopySplit/$c)"
...
read -p "mart: Ti copierò nella iso (in sola lettura) i file espressi con
l'opzione --scl, e che puoi visionare in /tmp/valeparted-listOfFilesToCopy.
Tale copia potrebbe richiedere del tempo.
