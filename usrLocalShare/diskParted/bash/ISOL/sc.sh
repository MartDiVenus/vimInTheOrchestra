...
/usr/local/share/valentine/diskParted/bash/ISOL/valeparted02ISOL.sh
...
### Siccome tale script fa parte di un ciclo, per non perdere tempo nel
## rispondere a ogni iterazioni, forzo con writeable, in mancanza di -w o -r.
scelta=1
...
## essendo tale script elemento di un ciclo, non chiedo l'etichetta per non perdere
# tempo a ogni iterazione.
...
sudo mkfs.$filesystem -F $myPart 2> /dev/null
...
#per sicurezza ti smonterò tutto, e copierò il file in
#$percorsoIsolato.
#
...
sha1sum $fileName.iso | awk '$1 > 0 {print $1}' > /tmp/valeparted-integrity01

cp $fileName.iso $percorsoIsolato

sync

if [ -f /tmp/valeparted-delCopiedFiles ]; then

	## Verifico che la copia sia avvenuta con successo
	if [ -f $percorsoIsolato/$fileName.iso ]; then

sha1sum $percorsoIsolato/$fileName.iso | awk '$1 > 0 {print $1}' > /tmp/valeparted-integrity02

comm -3 /tmp/valeparted-integrity01 /tmp/valeparted-integrity02 > /tmp/valeparted-integrityVerifica

byteVerifica=$(cat /tmp/valeparted-integrityVerifica)


if test $byteVerifica -eq 0

then
	echo "Copia avvenuta con successo, rimozione di:
$fileToCopy
$fileName.iso"
...
rm $fileName.iso
	...
	echo "La copia non è stata effettuata con successo, non è stata
	verificata l'integrità tra
$fileName.iso
e
$percorsoIsolato/$fileName.iso
"
fi
