...
/usr/local/share/valentine/diskParted/bash/Loop/valeparted02Loop.sh
...
1. Vuoi creare una partizione logica (gpt) per il disco loop

2. Vuoi eliminare una partizione logica (gpt) del disco loop. 
...
if [ -f /tmp/valeNetrw-devLoopList ]; then
	...
	cat /tmp/valeNetrw-devLoopList02 | head -n1 > /tmp/valeNetrw-devLoopListEg
...
echo "mart: Specifica il disco/partizione:
e.g.

$(cat /tmp/valeNetrw-devLoopListEg)"

read -p "
:" devicePart

echo "$devicePart" > /tmp/valeParted-devicePart

cat /tmp/valeParted-devicePart | sed 's/sd.//g' > /tmp/valeParted-nPart


myPart="$(cat /tmp/valeParted-devicePart)"
...
#sleep 3
	...
	echo " "
	echo " "
	echo "Non hai dischi/partizioni loop da eliminare.
	Uscita forzata."
