   ...
   do
    ...
    if test $leggoBytes -gt 0
     ...
     then
      ...
      ## controllo che sia un disco loop o meno
#ls /dev/sd* > /tmp/valeNetrw-lsDevLoop

#mkdir /tmp/valeNetrw-partLoop

grep $leggoB /tmp/valeNetrw-lsDevLoop > /tmp/valeNetrw-loopOrNot

stat --format %s /tmp/valeNetrw-loopOrNot > /tmp/valeNetrw-loopOrNotBytes

leggoBytesLoopOrNot=$(cat /tmp/valeNetrw-loopOrNotBytes)

if test $leggoBytesLoopOrNot -gt 0

then
	cat /tmp/valeNetrw-lsDevLoop | sed '/^'$leggoB'/d' > /tmp/valeNetrw-lsDevLoopSed

	cp /tmp/valeNetrw-lsDevLoopSed /tmp/valeNetrw-lsDevLoop

else

	echo $leggoB >> /tmp/valeNetrw-partLoop
fi
...
mkdir -p /tmp/ValeParted/$leggoA/$leggoC 2> /dev/null
		   
      sudo blkid /dev/$leggoC > /tmp/ValeParted/$leggoA/$leggoC/info

      echo " " >> /tmp/ValeParted/$leggoA/$leggoC/info

      ## No df per dischi non montati perché non sarebbe veritiero il responso.
      ## df -h /dev/$leggoC >> /tmp/ValeParted/$leggoA/$leggoC/info


      cat /tmp/valeNetrw-ValeDiskTree | sed 's/\|\_\_\/dev\/'$leggoC'//g' > /tmp/valeNetrw-ValeDiskTree2

      rm /tmp/valeNetrw-ValeDiskTree
                    
      cp /tmp/valeNetrw-ValeDiskTree2 /tmp/valeNetrw-ValeDiskTree

      ls -l /dev/$leggoC > /tmp/valeNetrw-BGrepped-umount-back

      cat /tmp/valeNetrw-BGrepped-umount-back | sed 's/-/din/g' > /tmp/valeNetrw-BGrepped-umount-back01
      cat /tmp/valeNetrw-BGrepped-umount-back01 | awk '$1 > 0 {print $5}' > /tmp/valeNetrw-dim
				  
      leggoDimensioni="$(cat /tmp/valeNetrw-dim)B"

	   sudo blkid /dev/$leggoC > /tmp/valeNetrw-blkidOut1
	   ...
	   leggoCytes=$(cat /tmp/valeNetrw-bytes)

	   if test $leggoCytes -gt 0
	   ...
	   ## il codice dei dischi non loop è 
	   ## echo "  |__/dev/$leggoC/ $leggoPartLabel $leggoPartType $leggoDimensioni unmounted" >> /tmp/valeNetrw-ValeDiskTree	
	   echo "  |__/dev/$leggoC/ $leggoPartLabel $leggoPartType $leggoDimensioni loop" >> /tmp/valeNetrw-ValeDiskTree		   
fi
done
