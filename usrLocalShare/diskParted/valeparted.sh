...
split -l 1 /tmp/real1 splitReal/

for a in $(ls splitReal/)

 do
	 
	 leggoA1="$(cat splitReal/$a)"
...
for b in $(ls splitReal/)

 do
   
   leggoMountedGenericPart="$(cat splitReal/$b)"
...
1. primaria (max 4 partizioni primarie)

2. estesa (container per partizioni logiche)
...
## fdisk erroneamente usa le virgole al posto del pto., per le cifre decimali.
...
then
 ...
 cat /tmp/valeparted-nPartioneOut01 | awk '$1 > 0 {print $6}' > /tmp/valeparted-nPartioneDefault
...
## * credo valga anche per le partizioni logiche.
